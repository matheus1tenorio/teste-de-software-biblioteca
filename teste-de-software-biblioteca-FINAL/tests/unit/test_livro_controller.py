...
def test_criar_livro():
    ...
    # Mock do get_all_livros para retornar um livro com ID
    mock_livros = [{"id": 42, "titulo": "Duna"}]

    with patch(
        "livro_service.controllers.livro_controller.get_all_livros",
        return_value=mock_livros
    ), patch(
    ...
    assert result == (
        {"mensagem": "Livro criado com sucesso", "id": 42},
        201
    )
...
def test_editar_livro_inexistente_retorna_erro():
    """Teste de Falha de Negócio: Tenta editar livro que não existe"""
    dados = {
    ...
    with patch(
        ...
        result = editar_livro(9999, dados)
    
    assert result[1] == 404
    assert "Livro não encontrado" in result[0]["erro"]