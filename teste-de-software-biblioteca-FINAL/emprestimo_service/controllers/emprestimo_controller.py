...
def criar_emprestimo(data):
    ...
    # === NOVO: Busca o ID do empréstimo recém-criado ===
    #from models.emprestimo_model import get_all_emprestimos
    todos_emprestimos = get_all_emprestimos()
    
    if todos_emprestimos:
        ultimo_emprestimo = max(todos_emprestimos, key=lambda x: x["id"])
        emprestimo_id = ultimo_emprestimo["id"]
    else:
        emprestimo_id = None
